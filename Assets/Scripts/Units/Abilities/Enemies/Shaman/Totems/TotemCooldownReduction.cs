...
public class TotemCooldownReduction : Ability
{
	int cooldownMod = 1;
	
	public TotemCooldownReduction (Unit u, TileMap m, PrefabLibrary el) : base(u, m , el)
	{
		range = 6;
		duration = 1;
		area = AreaType.Self;
	...
	public override void UseAbility (Node target)
	{
		...
		foreach (Node n in target.reachableNodes) {
			...
			if (sUnit.isActive && sUnit != myCaster) {
				sUnit.ApplyEffect( new CooldownEffect("Wind Totem", duration, cooldownMod));
			}
		}
	}
}
